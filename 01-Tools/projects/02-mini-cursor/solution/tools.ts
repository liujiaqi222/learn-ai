...
const readFileTool = tool(async ({ filePath }: { filePath: string }) => {
    try {
        ...
        console.log(`[工具调用]read_file(${filePath})成功读取${fileContent.length}字节`);
...
// 2. 写入文件工具
const writeFileTool = tool(async ({ filePath, content }: { filePath: string, content: string }) => {
    try {
...
// 4.列出目录内容
const listDirectoryTool = tool(async ({ directoryPath }: { directoryPath: string }) => {
    try {
        const files = await fs.readdir(directoryPath)
        console.log(`[工具调用] list_directory("${directoryPath}") - 找到 ${files.length} 个项目`);
        return `目录内容: ${files.toString()}`
    } catch (err) {
        console.log(`  [工具调用] list_directory("${directoryPath}") - 错误: ${err}`);
        return `列出目录失败: ${err}`;
    }
